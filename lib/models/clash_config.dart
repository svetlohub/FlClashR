...
class Tun with _$Tun {
  ...
  factory Tun.safeFormJson(Map<String, Object?>? json) {
    if (json == null) return defaultTun;
    try {
...
class GeoXUrl with _$GeoXUrl {
  ...
  factory GeoXUrl.safeFormJson(Map<String, Object?>? json) {
    if (json == null) return defaultGeoXUrl;
    try {
...
class ClashConfig with _$ClashConfig {
  ...
  factory ClashConfig.safeFormJson(Map<String, Object?>? json) {
    if (json == null) return defaultClashConfig;
    try {
...
// ─── OverrideData / OverrideRule ──────────────────────────────────────────────
// Defined here (not in profile.dart) so that core.dart can import clash_config.dart
// without a circular dependency through profile.dart.
...
extension OverrideRuleExt on OverrideRule {
  ...
  List<Rule> getRules(List<Rule> originRules) {
    return switch (type) {
      ...
      OverrideRuleType.override => [
          ...addedRules,
          ...originRules
              .map((rule) => overrideRules
                  .firstWhere((or) => or.id == rule.id, orElse: () => rule))
              .toList(),
        ],
    };
  }
  ...
  OverrideRule updateRules(List<Rule> Function(List<Rule> rules) builder) {
    return switch (type) {
      OverrideRuleType.added => copyWith(addedRules: builder(addedRules)),
      OverrideRuleType.override =>
